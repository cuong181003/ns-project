   ...
   public static void paint(mGraphics var0) {
      ...
      if (info != null) {
         if (GameCanvas.currentDialog == null || GameCanvas.currentDialog.center == null) {
            ...
            if (GameCanvas.isTouch) {
               Paint.paintFrame(var1, var2 - 4, var3 + 10, hI + 8, var0);
            } else {
               var0.setColor(0);
               var0.fillRect(var1 - 2, var2, var3 + 2, hI);
            }

            var0.setClip(var1, var2, var3, hI);
            info.f.drawString(var0, info.s, x, var2 + 3, 0);
         }
      }
   }
   ...
   public static void update() {
      if (p1 == 0) {
         ...
         if (infoWaitToShow.size() > 0) {
            ...
            info = var0;
            strWidth = var0.f.getWidth(info.s);
            p2 = 0;
            p1 = 0;
            x = GameCanvas.w;
            return;
